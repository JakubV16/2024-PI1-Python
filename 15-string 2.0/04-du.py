# uzivatel zada celu vetu a program vetu rozdeli na slova , ktore vypise nahodnou farbou na nahodnom mieste okej !!!
# aj písmenka zo slov vypise nahodnymi farbami
...
def start():
    ...
    if entered_text:
        ...
        #     canvas.create_text(random.randint(20,680),random.randint(20, 380), text=char, fill=random_color(), font=("arial",18))
        for word in words:
            x = random.randint(75,625)
            y = random.randint(20, 380)
            # canvas.create_text(random.randint(75,625),random.randint(20, 380), text=word, fill=random_color(), font=("arial",18))
            for char in word:
